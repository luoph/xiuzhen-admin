    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="详情id" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number :disabled="true" v-decorator="['lotteryDetailId', validatorRules.lotteryDetailId]"
                          placeholder="请输入详情id" style="width: 100%"/>
        </a-form-item>
        <a-form-item label="积分" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="['score', validatorRules.score]" placeholder="请输入积分" style="width: 100%"/>
        </a-form-item>
        <a-form-item label="奖励列表" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['reward', validatorRules.reward]"
                   placeholder='请输入奖励列表 e.g. [{"itemId":1001, "num":1}]'></a-input>
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
...
export default {
  name: "OpenServiceCampaignLotteryDetailScoreModal",
  components: {
    JDate
  },
  data() {
    return {
      ...
      url: {
        add: "game/openServiceCampaignLotteryDetailScore/add",
        edit: "game/openServiceCampaignLotteryDetailScore/edit"
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    edit(record) {
      ...
      console.log("OpenServiceCampaignLotteryDetailScoreModal, model:", JSON.stringify(this.model));
      this.visible = true;

      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, "campaignId", "campaignTypeId", "lotteryDetailId", "score", "reward"));
      });
    },
    close() {
      this.$emit("close");
    ...
    popupCallback(row) {
      this.form.setFieldsValue(pick(row, "campaignId", "campaignTypeId", "lotteryDetailId", "score", "reward"));
    }
  }
};
</script>
