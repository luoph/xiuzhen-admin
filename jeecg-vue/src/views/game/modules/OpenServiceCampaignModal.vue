  ...
  methods: {
    ...
    edit(record) {
      ...
      this.$nextTick(() => {
        ...
        this.form.setFieldsValue(pick(this.model, 'name', 'cross', 'serverIds', 'autoAddServerChannels', 'icon', 'status', 'autoOpen', 'remark', 'priority'));
      });
    },
    close() {
      this.$emit('close');
    ...
    popupCallback(row) {
      this.form.setFieldsValue(pick(row, 'name', 'cross', 'serverIds', 'autoAddServerChannels', 'icon', 'status', 'autoOpen', 'remark', 'priority'));
    },
    changeSelect(value) {
