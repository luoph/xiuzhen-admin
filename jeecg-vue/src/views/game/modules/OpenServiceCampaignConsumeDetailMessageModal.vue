    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="传闻推送时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-date-picker placeholder="传闻推送时间" showTime format="YYYY-MM-DD HH:mm:ss"
        ...
        <a-form-item label="传闻内容" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-textarea v-decorator="['message', validatorRules.message]" placeholder="请输入传闻内容"/>
        </a-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        add: 'game/openServiceCampaignConsumeDetailMessage/add',
        edit: 'game/openServiceCampaignConsumeDetailMessage/edit'
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    popupCallback(row) {
      this.form.setFieldsValue(pick(row, 'campaignId', 'campaignTypeId', 'consumeDetailId', 'sendTime', 'message', 'num', 'email'));
    }
  }
};
</script>
