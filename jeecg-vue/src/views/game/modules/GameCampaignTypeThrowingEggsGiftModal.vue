    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="礼包名" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['giftName', validatorRules.giftName]" placeholder="请输入礼包名" />
        </a-form-item>
        ...
        <a-form-item label="限购条件" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['limitCondition', validatorRules.limitCondition]" placeholder="请输入限购条件" />
        </a-form-item>
        <a-form-item label="奖励内容" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['reward', validatorRules.reward]" placeholder='奖励内容:[{"itemId":1001,"num":1001}]' />
        </a-form-item>
...
export default {
  ...
  methods: {
    ...
    edit(record) {
      ...
      this.$nextTick(() => {
        this.form.setFieldsValue(
          pick(this.model, 'campaignId', 'typeId', 'costItemId', 'costNum', 'stack', 'amount', 'discount', 'limitCondition', 'reward', 'giftName', 'minLevel', 'maxLevel')
        );
      });
    },
    close() {
    ...
    popupCallback(row) {
      this.form.setFieldsValue(
        pick(row, 'campaignId', 'typeId', 'costItemId', 'costNum', 'stack', 'amount', 'discount', 'limitCondition', 'reward', 'giftName', 'minLevel', 'maxLevel')
      );
    }
  }
};
