    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-radio-group @change="contentType($event.target.value)" v-decorator="['emailType', { initialValue: 1 }, validatorRules.emailType]" style="width: 100%">
        ...
        <a-form-item label="目标类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-radio-group @change="selectTarget($event.target.value)" v-decorator="['targetBodyType', { initialValue: 1 }]" dict style="width: 100%">
        ...
        <a-form-item v-if="playerType" label="玩家ID" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-textarea v-decorator="['targetBody', { initialValue: '' }]" placeholder="请以英文“,”分割输入多个玩家ID" style="width: 100%" :autoSize="{ minRows: 2, maxRows: 6 }" />
        </a-form-item>
        <a-form-item v-if="serverType" label="区服ID" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <game-server-selector v-decorator="['targetBodyIds', { initialValue: '' }]" @onSelectServer="change" />
        </a-form-item>
        ...
        <a-form-item label="开始时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-date-picker placeholder="请选择开始时间" showTime format="YYYY-MM-DD HH:mm:ss" v-decorator="['validStarTime', validatorRules.validStarTime]" style="width: 100%" />
        </a-form-item>
        <a-form-item label="结束时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-date-picker placeholder="请选择结束时间" showTime format="YYYY-MM-DD HH:mm:ss" v-decorator="['validEndTime', validatorRules.validEndTime]" style="width: 100%" />
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
...
export default {
  ...
  methods: {
    ...
    edit(record) {
      ...
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, 'id', 'title', 'remark', 'emailType', 'content', 'validState', 'targetBodyType', 'targetBodyIds', 'targetBody'));
    ...
    popupCallback(row) {
      this.form.setFieldsValue(
        pick(row, 'id', 'title', 'remark', 'emailType', 'content', 'validState', 'targetBodyType', 'targetBodyIds', 'sendTime', 'validStarTime', 'validEndTime', 'targetBody')
      );
    },
    selectTarget(e) {
      // 1-玩家 2-服务器
