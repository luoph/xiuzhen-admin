    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="排名最小值" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="['minRank', validatorRules.minRank]" placeholder="请输入排名最小值"
                          style="width: 100%"/>
        </a-form-item>
        <a-form-item label="排名最大值" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="['maxRank', validatorRules.maxRank]" placeholder="请输入排名最大值"
                          style="width: 100%"/>
        </a-form-item>
        <a-form-item label="上榜最低积分" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="['score', validatorRules.score]" placeholder="请输入上榜最低积分"
                          style="width: 100%"/>
        </a-form-item>
...
export default {
  name: "OpenServiceCampaignLotteryDetailRankingModal",
  components: {
    JDate
  },
  data() {
    return {
      ...
      url: {
        add: "game/openServiceCampaignLotteryDetailRanking/add",
        edit: "game/openServiceCampaignLotteryDetailRanking/edit"
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    popupCallback(row) {
      this.form.setFieldsValue(pick(row, "campaignId", "campaignTypeId", "lotteryDetailId", "minRank", "maxRank", "score", "reward"));
    }
  }
};
</script>
