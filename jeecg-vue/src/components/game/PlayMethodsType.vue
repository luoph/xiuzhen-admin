<template>
  <div>
    <a-form-item label="玩法类型" :label-col="{ span: 12 }" :wrapper-col="{ span: 16 }">
      <a-select placeholder="请选择玩法类型" v-model="playMethodsType" :initialValue="playMethodsType"
                @change="onSelectPlayMethodsType">
        <a-select-option v-for="playMethodsType in playMethodsTypeList" :key="playMethodsType.name"
                         :value="playMethodsType.type">
