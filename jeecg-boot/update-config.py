...
def copy_json(svn_url, source_folder, target_folder, config_list, other_config=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--svn', type=str, default='svn://dev.1101stu.com:7690')
    parser.add_argument('--profile', type=str, default='develop')
    args = parser.parse_args()

    # 查找 RefMgr.ts
    config_file_path = ''
    for root, dir_list, file_list in os.walk('jeecg-boot-module-system'):
        for filename in fnmatch.filter(file_list, "GameConfig.java"):
    ...
    copy_json(repo_url, 'xiuzhen-config', 'jeecg-boot-module-system/src/main/resources/json', all_config)
