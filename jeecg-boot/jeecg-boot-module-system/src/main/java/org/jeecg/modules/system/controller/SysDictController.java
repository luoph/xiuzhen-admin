...
public class SysDictController extends JeecgController<SysDict, ISysDictService> {
    ...
    public Result<List<String>> loadDictItem(@PathVariable("dictCode") String dictCode,
                                             @RequestParam(name = "key") String keys,
                                             @RequestParam(value = "sign", required = false) String sign,
                                             @RequestParam(value = "delNotExist", required = false, defaultValue = "true") boolean delNotExist, HttpServletRequest request) {
    ...
    public ModelAndView exportXls(SysDict sysDict, HttpServletRequest request) {
        QueryWrapper<SysDict> queryWrapper = prepareQuery(sysDict, request);
        List<SysDict> sysDictList = sysDictService.list(queryWrapper);
        List<SysDictPage> pageList = new ArrayList<SysDictPage>(sysDictList.size());
        ...
        return ExcelUtils.exportXls(sysUser.getRealname(), pageList, request.getParameter("selections"), SysDictPage.class, "数据字典");
    }
