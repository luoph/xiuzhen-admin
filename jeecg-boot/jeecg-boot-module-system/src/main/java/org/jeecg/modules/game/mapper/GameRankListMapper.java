...
public interface GameRankListMapper {
     ...
     */
    @Select("select player_id, count(*) as value from ${logPlayerTable} where type = #{type} and server_id = #{serverId} and create_time <= #{createTime} group by player_id having count(*) >= 1 ORDER BY count(*) desc")
     ...
     */
    @Select("select player_id, max(value) as value from ${logPlayerTable} where type = #{type} and server_id = #{serverId} and create_time <= #{createTime} group by player_id  ORDER BY max(value) desc")
