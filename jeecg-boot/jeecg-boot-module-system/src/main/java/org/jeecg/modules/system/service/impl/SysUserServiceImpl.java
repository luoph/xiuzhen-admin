...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public SysRoleIndex getDynamicIndexByUserRole(String username, String version) {
        ...
        if (oConvertUtils.isNotEmpty(version) && roles.size() > 0) {
...
//		this.save(user);  //保存角色的时候已经添加过一次了
    ...
    public void editUserWithDepart(SysUser user, String departs) {
        ...
        //查询已关联部门
        List<SysUserDepart> userDepartList = sysUserDepartMapper.selectList(new QueryWrapper<SysUserDepart>().lambda()
        ...
        if (userDepartList != null && userDepartList.size() > 0) {
            for (SysUserDepart depart : userDepartList) {
                ...
                if (!Arrays.asList(arr).contains(depart.getDepId())) {
                    ...
                    List<String> roleIds = sysDepartRoleList.stream().map(SysDepartRole::getId).collect(Collectors.toList());
                    if (roleIds.size() > 0) {
                        departRoleUserMapper.delete(new QueryWrapper<SysDepartRoleUser>().lambda()
