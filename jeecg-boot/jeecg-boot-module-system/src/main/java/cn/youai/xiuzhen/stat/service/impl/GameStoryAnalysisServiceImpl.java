...
import cn.youai.xiuzhen.stat.mapper.GameStoryAnalysisMapper;
import cn.youai.xiuzhen.stat.service.IGameStoryAnalysisService;
    ...
    private List<GameStoryAnalysisVO> genGameStoryAnalysisList(GameStoryAnalysisVO gameStoryAnalysis,
        ...
        for (GameStoryAnalysisVO vo : result) {
            ...
            if (totalStayLiveNum != 0 && vo.getStayLiveNum() != null) {
                vo.setLiveRate(BigDecimalUtils.dividePercent(BigDecimalUtils.divide((double) vo.getStayLiveNum(), totalStayLiveNum, 4).doubleValue()).toString());
            } else {
                vo.setLiveRate("0");
            }
            // 流失占比
            if (totalStayLeaveNum != 0 && vo.getStayLeaveNum() != null) {
                vo.setLeaveRate(BigDecimalUtils.dividePercent(BigDecimalUtils.divide((double) vo.getStayLeaveNum(), totalStayLeaveNum, 4).doubleValue()).toString());
            } else {
                vo.setLeaveRate("0");
            }
            // 关卡滞留率
            if (vo.getTotalArriveNum() != null && vo.getTotalArriveNum() != 0 && vo.getTotalStayNum() != null) {
                vo.setCheckpointStayRate(BigDecimalUtils.dividePercent(BigDecimalUtils.divide((double) vo.getTotalStayNum(), (double) vo.getTotalArriveNum(), 4).doubleValue()).toString());
            } else {
