...
import cn.youai.xiuzhen.game.entity.PayOrderBill;
        ...
        for (String payRank : payRankList) {
            ...
            if (null == allPayInfoList) {
                ...
                if (0 == payOrderBill.getPayNumSum()) {
                    ...
                    payOrderBill.setArppu(BigDecimalUtils.divide(payOrderBill.getPayAmountSum().doubleValue(), payOrderBill.getPayNumSum(), 4));
                }
...
//            payOrderBill.setPayAmountSumRate();
    ...
    private PayOrderBill getDataTreating(PayOrderBill payOrderBill) {
        ...
        payOrderBill.setPayNumSumRate(BigDecimalUtils.dividePercent(payNumSumRate.doubleValue()));

        if (payAmountSumRate == null) {
            payAmountSumRate = BigDecimal.ZERO;
        }
        payOrderBill.setPayAmountSumRate(BigDecimalUtils.dividePercent(payAmountSumRate.doubleValue()));

        if (arppu == null) {
            arppu = BigDecimal.ZERO;
        }
        payOrderBill.setArppu(BigDecimalUtils.divide(arppu.doubleValue(), 1, 3));
