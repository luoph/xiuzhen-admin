...
import cn.youai.xiuzhen.game.entity.GamePlayerItemLog;
import cn.youai.xiuzhen.game.mapper.GamePlayerItemLogMapper;
import cn.youai.xiuzhen.game.service.IGamePlayerItemLogService;
...
public class GamePlayerItemLogServiceImpl extends ServiceImpl<GamePlayerItemLogMapper, GamePlayerItemLog> implements IGamePlayerItemLogService {
    ...
    public List<GamePlayerItemLog> queryCurrencyPayIncomeList(String rangeDateBegin, String rangeDateEnd, int days, Integer serverId, int itemId) {
        ...
        for (GamePlayerItemLog incomeItemLog : incomeList) {
            ...
            // 消耗比
            BigDecimal consumeRate = BigDecimalUtils.divideFour(addItemNum.doubleValue(), consumeItemNum.doubleValue(), true);
            incomeItemLog.setConsumeRate(consumeRate);
            // 滞留
            BigDecimal retention = BigDecimalUtils.subtract(addItemNum.doubleValue(), consumeItemNum.doubleValue());
        ...
        consumeRateSumBigDecimal = BigDecimalUtils.divideFour(addItemNumSum, consumeItemNumSum, true);
        retentionSumBigDecimal = BigDecimalUtils.subtract(addItemNumSum, consumeItemNumSum);
