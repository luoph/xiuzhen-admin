<template>
  <a-card :bordered="false">
    <a-table rowKey="id" bordered :columns="columns" :dataSource="dataSource" :pagination="false"></a-table>
  </a-card>
</template>
        ...
        if (![key, dataIndex].includes(numKey)) {
          ...
          dataSource.forEach(data => {
            ...
            console.log(
              data[dataIndex],
              ':',
              /^\d+\.?\d?$/.test(data[dataIndex]) ? Number.parseInt(data[dataIndex]) : Number.NaN
            )
          })
